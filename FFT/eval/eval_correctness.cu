                      ...
                      bool toFile, std::string filename)
{
    ...
    for (int i = 0; i < rows; i++)
        for (int k = 0; k < depth; k++)
            for (int j = 0; j < truncated_cols; j++)
                all_vecs[i*depth*truncated_cols + k*truncated_cols  + j] = ds.getElement(i+1, j+1, k+1);
    ...
    for (int i = 0; i < 1; i++) // **** change 1 to rows
        for (int k = 0; k < 1; k++) // **** change 1 to depth
            for (int j = 0; j < truncated_cols; j++)
                fftFile << "Channel " << i << ", FFT[" << j <<"] = " << all_vecs[i*depth*truncated_cols + k*truncated_cols + j] << endl;
